...
3. A review table prints (one row per pool + schedule), and one zip downloads automatically: `nme-autoscale-profiles-<timestamp>.zip` — the HTML, the review CSV, the raw plan/pool data, and the run's console log. If someone asked you to run this, that zip is the only thing to send back.
