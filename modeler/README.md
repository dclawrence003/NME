...
| A workspace prints `BLOCKED BY ITS NETWORK SETTINGS` | The workspace only accepts queries from approved networks (Network Security Perimeter / public query access off / private link) — not a permissions problem. Run the same command from **inside the customer network** (local PowerShell on a VPN/corp machine), or allow the runner's IP in the workspace's networking settings |
