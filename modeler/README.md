...
8. **Actual spend.** Last calendar month's cost for the session-host VMs and OS disks via the Cost Management Query API: one query per subscription, filtered to the resource groups that matter. Attributed by resource id first, then by resource group where last month's hosts no longer exist (rebuilt fleets), split by host count where a group is shared. If Azure throttles, the script waits as long as Azure asks and tries again; anything still refused gets a second pass after a cooldown. See the cost section for the details.
...
**How spend is attributed to a pool.** First by resource id: last month's rows for the pool's current VMs and their OS disks. That alone is not enough, because some environments rebuild every session host on each release, and last month's VMs then no longer exist. A 47-pool run once attributed $7.3k of $76.3k that way, with nine production pools reading zero while their spend sat in "other". So, second, VM and disk spend in a resource group that holds one pool's hosts belongs to that pool whether or not the ids match; the pool is flagged `hosts rebuilt since the billing month` and `ActualBasis` says `by id + resource group`. Third, a resource group shared by several pools splits its unmatched VM and disk spend by current host count, flagged as an estimate. Storage accounts never enter a pool's total. rawdata.json carries a `costAttribution` block with the three parts per pool.
